...
<link rel="stylesheet"
	href="/static/plugins/fullcalendar/fullcalendar.min.css" />
<style type="text/css">
.hanbit-header {
	position: fixed;
	width: 100%;
	...
	<script type="text/javascript">
		$(document).ready(
				function() {
					...
					var range = {
						start : currentMoment.startOf("month").format(
								"YYYYMMDDHHmm"),
						end : currentMoment.endOf("month").format(
								"YYYYMMDDHHmm")
					};
					...
					function getMonthlySchedules() {
						range.start = currentMoment.startOf("month").format(
								rangeFormat);
						range.end = currentMoment.endOf("month").format(
								rangeFormat);
							...
							data : {
								...
								function(data) {
									for (var i = 0; i < data.length; i++) {
										...
										event.start = moment(data[i].startDt,
												rangeFormat).format(
												"YYYY-MM-DDTHH:mm");
										event.end = moment(data[i].endDt,
												rangeFormat).format(
												"YYYY-MM-DDTHH:mm");
